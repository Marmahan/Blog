...
app.post('/evaluate', function(req,res){
    ...
    //this number should be equal or bigger than the number duplicated services for each service
    //so if we have 3 validation services, then this number should be 3 or bigger
    ...
    /************************************************************************************** */ 
                ...
                console.log('Here the port value is '+portvalue+' reqport is '+reqPort);
                ...
                /////////////WORKING
                 Relation.findOneAndUpdate(
                     {port: portvalue, 'services.port':reqPort }, 
                     {'$set': {'services.$.trust': trustvalue}}, { new: true, useFindAndModify: false })
                     .then(()=>{
            ...
            //we should select the max trust
                        ...
                        //WORKING
                        Relation.findOneAndUpdate(
                            {port: results[indexOfMaxValue].port, 'services.port':reqPort }, 
                            {'$set': {'services.$.trust': results[indexOfMaxValue].trust}}, { new: true, useFindAndModify: false })
                            .then(()=>{
                           console.log('direct trust updated from here');
                       }).catch((err)=>{
                           console.log(err);
                       })
                ...
                //should send the highest value out of failed attempts
                        ...
                        Relation.findOneAndUpdate(
                            ...
                            {'$set': {'services.$.trust': results[indexOfMaxValue].trust}}, { new: true, useFindAndModify: false })
                            .then(()=>{
                           console.log('direct trust updated after no more tries');
                       }).catch((err)=>{
                           console.log(err);
                       })
                        /////////here/////////////
